...
$index_file = "member.php";
 ...
 $template->set_filenames(array(
		...
		switch($op)
		{
			...
			case "member":
				Detailmember($mem_id,$MEMBER);
				break;
			case "pearls":
				Pearls($mem_id,$MEMBER);
				break;
			case "balance":
				Balance($mem_id,$MEMBER);
				break;
			case "rating":
				Rating($mem_id,$MEMBER);
				break;
						case "view":
			View($bid);
			break;
		case "ratingcode":
			RatingCode($code,$MEMBER);
			break;
			case "libraly":
				Libraly($mem_id,$MEMBER);
				break;
				    //#####   library #########
		case "savelibrary":
			$file = SaveFile("file",$SaveFilePath,"lib");
			Savelibrary($lib,$mem_id,$pdate,$name,$file,$remark,$C1);
			break;
		case "inputlibrary":
			Inputlibrary($lib);
			break;
...
function Libraly($mem_id,$MEMBER)
{
	...
	$template->assign_vars(array(
      'PAGE_LIST'=>List_PageStr($page,$totalpage,$link),
		'JAVA_URL'=>"$index_file?m=$module_name&op=del&d=$MEMBER",
		'U_ADD'=>"$index_file?m=$module_name&op=inputlibrary&gr=$gr&d=$MEMBER"
		)	);
...
function Del($tb,$key,$id,$bk,$rt,$page)
{
  ...
  global $mem_id;
		...
		if( $tb=="library"){
		DeleteFileDB($tb,"file", " and $key=$id", $SaveFilePath);
		}

		$db->sql_query($query);
			if(!empty($rt))
			{
			$goto = "$index_file?m=$module_name&op=$rt&gr=$gr&mem_id=$mem_id&d=".$_GET['d'];
			}else{
			$goto="$index_file?m=$module_name&op=game&page=$page&gr=$gr&mem_id=$mem_id&d=".$_GET['d']."#$bk";
			}
...
function Savelibrary($lib,$mem_id,$pdate,$name,$file,$remark,$C1)
{
  global $admin,$db,$index_file,$module_name,$SaveSMPath,$SaveBGPath,$SaveFilePath,$gr;
		...
		if(empty($lib))
		{
		$field="lib,mem_id,pdate,name,file,remark";
		$data="NULL,'$mem_id','$pdate','$name','$file','$remark'";
		SaveDB( $table,$field,$data);
		}else{
		$dataset="mem_id='$mem_id' ,pdate='$pdate' ,name='$name'  ,remark='$remark' ";
		$condition=" lib='$lib' ";
	    if(!empty($file) || !empty($C1))
			{
			DeleteFileDB($table,"file", " and $condition", $SaveFilePath);
			$dataset.=" ,file='$file' ";
			}
			//echo $C1.":".$file;
		...
		Header("Refresh: 0;url=$index_file?m=$module_name&op=libraly&gr=$gr");

}



function Inputlibrary($lib)
{
  global $admin,$db,$index_file,$module_name,$SaveSMPath,$SaveBGPath,$SaveFilePath, $template,$mainframe,$adminframe,$gr;
  global $mem_id;
	...
	if(!empty($gr))
		{
	 $module_file = "modules/$module_name/$gr/inputlibrary.php";
	 $template->SetImagePath("modules/$module_name/$gr/");
		}else{
	 $module_file = "modules/$module_name/inputlibrary.php";
	$template->SetImagePath("modules/$module_name/");
		}
...
function Rating($mem_id,$MEMBER)
{
	...
	if(empty($page))
		...
		$edit="$index_file?m=$module_name&op=ratingcode&code=$code&gr=$gr&d=".$_GET['d'];
		$cap=str_replace("","",$cap);
...
function RatingCode($code,$MEMBER)
{
  ...
  global $mem_id,$username,$password,$member_ofck,$name,$member_name;
	...
	$template->assign_vars(array(
	  ...
	  'MEMBER_NAME'=>$member_name,
		'LOGOUT'=>"index.php?m=member&op=logout&d=".$_GET['d'],
		'EDIT_PROFILE'=>"member.php?m=member&op=inputmember&d=".$_GET['d'],
		'HISTORY'=>"member.php?m=history&d=".$_GET['d'],
		'COMMUNITY'=>"member.php?m=community&d=".$_GET['d'],
		'PEARSL'=>"member.php?m=fearsl&d=".$_GET['d'],
		'BENCH'=>"member.php?m=bench&d=".$_GET['d'],
		)	);
...
$c=1;
  $result = $db->sql_query($sql);
    ...
    $view = "index.php?m=$module_name&op=view&bid=$bench_id&country=$bench_country[$bench_id]&d=".$_GET['d'];
	$template->assign_block_vars('listrow', array(
		'BENCH_ID'=>$bench_id,
		'MEM_ID'=>$smem_id,
		'NAME'=>$member_name,
...
function Balance($mem_id,$MEMBER)
{
	global $admin,$db,$index_file,$module_name,$SaveSMPath,$SaveBGPath,$SaveFilePath, $template,$mainframe,$adminframe,$gr;
	global $page;
	...
	if(!empty($gr))
	 ...
	 $table="bench";
			$fieldArr =" * ";
			$searchkey=" and mem_id='$mem_id'  order by bench_id desc";
		...
		while($v= $db->sql_fetchrow($result) )
		{
				...
				$cap=str_replace("","",$balance_sheet);
				$del="javascript:Del('tb=bench&key=bench_id&id=$bench_id&op=del&rt=balance&mem_id=$mem_id&gr=$gr','$cap')";
				$DelArr[]=$del;
...
//echo "Hey Assign";
	...
	for($i=0;$i< $cnt_row;$i++)
	{
		...
		$var2 = "D".(5-$cnt_row+$i+1);

		$template->assign_vars(array(
			$var=>$DateArr[$i],
			));
				$template->assign_block_vars($var2, array(		
				'UD'=>$DelArr[$i],
				)	);
		...
		$link="$index_file?m=$module_name&op=balance&d=$MEMBER";

	if($page > 1 ){

		$bk_link="<a href=\"$link&page=".($page-1)."\">";
		$bk_link.=" <img border=\"0\" src=\"images/back.gif\" width=\"24\" height=\"24\"></a>";    
		$template->assign_vars(array('BK'=>$bk_link));
	}
		
	if(   ($page+1) <= $totalpage ){
		 $ffd_link="<a href=\"$link&page=".($page+1)."\">";
        $ffd_link.="<img border=\"0\" src=\"images/forward.gif\" width=\"23\" height=\"23\"></a>";
			$template->assign_vars(array('FFD'=>$ffd_link));
	}
...
function Pearls($mem_id,$MEMBER)
{
	...
	if(!empty($gr))
	 ...
	 $table="bench_result";
			$fieldArr =" * ";
			$searchkey=" and mem_id='$mem_id'  order by resu desc";
			//$result=SearchDB( $searchkey,$table,$fieldArr);
		...
		$cap=str_replace("","",$date);
		 $del="javascript:Del('tb=bench_result&key=resu&id=$resu&op=del&rt=pearls&mem_id=$mem_id&gr=$gr','$cap')";
	...
	for($i=0;$i< $cnt_row;$i++)
	{
		...
		$link="$index_file?m=$module_name&op=pearls&d=$MEMBER";
	...
	//echo " total=$totalpage :page = $page";
...
function Detailmember($mem_id,$MEMBER)
{
  global $admin,$db,$index_file,$module_name,$SaveSMPath,$SaveBGPath,$SaveFilePath, $template,$mainframe,$adminframe,$gr;
						$title="";
				$firstname="";
